% Make sure to extract tar.gz files to .nc before running
clear all
close all
...
try % addded
 lat{BB}=ncread(nh,'lat'); % added (BB,:) changed to {BB}
...
BB=BB+1;
        ...
        warning('Skipped file %s due to missing variable: %s', nh, ME.message);
        continue;
...
% convert lat lon year mon day into doubles instead of cells
lat = cell2mat(lat) ;
lon = cell2mat(lon) ;
year = cell2mat(year) ;
...
save('C:\Users\ajs82292\Desktop\Research\Matlab\Source\nodc_cleaning\01Greenland_temporary.mat','lat','lon','temp','sal','depth','year','month','day')
...
save('C:\Users\ajs82292\Desktop\Research\Matlab\Source\nodc_files_data\01GREENLAND_temp_sal','yea','mon','day','lon','lat','dep','watdep','temp','sal')
